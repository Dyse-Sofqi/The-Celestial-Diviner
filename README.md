...
- **键盘注入 4 模式**（底部下拉框热切换）：
  ...
  - **DD 驱动（物理级）**：经 DD 虚拟驱动注入，**不带 `LLKHF_INJECTED` 标记**，
    ...
    （**默认模式**；需程序目录有 `dd63330.dll`，官方免费版加载时需联网授权；
    失败自动回退普通模式）
- **多目标绑定**：每个输入源（键盘键 / 鼠标 7 键 / 滚轮）可绑定多个目标键，独立配置：
  - 目标键（键盘键 / 鼠标键 / 滚轮）
  - 模式：**开关 Toggle**（按一次启动 / 再按停止）或 **按压 Hold**（按住连发，松开即停）
  - 间隔：**10 ~ 100ms，默认 10ms**（`timeBeginPeriod(1)` 保证定时精度；
    添加连发键时按 10ms 默认生成，可在方案列表行内文本框修改，回车 / 失焦提交，越界自动钳制）
...
- **总开关（全局开关键）**：默认键 **F9**（可自定义），作为所有方案的按键总开关：
  ...
  - 提示音用 Windows 内置中文语音离线合成（内嵌 WAV），启动时解包到临时目录播放；
    程序自身不依赖任何 TTS 服务，音量由状态栏滑块控制
...
- **界面**：简体中文；深浅主题跟随系统；Per-Monitor V2 DPI 感知；
  键盘区按真实 ANSI 键宽与导航簇位置拟真排布（方向键倒 T 形）
- 状态颜色：未注册浅灰 / Toggle 蓝 / Hold 橙；**总开关开启时全部控件暗淡**（面板仅供观察），
  关闭时恢复清晰（便于编辑方案）+ 黄色横幅
...
3. 对话框内"+ 添加" → "录制"（5 秒内按下目标键）→ 选模式 → "确认添加" → "确认"
...
本项目基于 **GPL-3.0** 发布（见 LICENSE）。

键位可视化悬浮层的键帽视觉复刻自 [keyviz](https://github.com/mulaRahul/keyviz)
（GPL-3.0）的 PBT 键帽设计；方案行可视化开关的 eye / eye-off 图标来自

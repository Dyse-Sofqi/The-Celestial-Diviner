...
- ⚠️ 本机 Change Box 自带 DD64.dll 不可用（更正 2026-09-03：非"时间锁过期"。
  它是转发壳，内部加载 DD202xLocal.x32.dll 并转发 DD_* 导出，但该核心 DLL 在本机
  不存在 → DD_btn(0) 恒 -1；DLL 无网络 API 导入，与在线校验无关。另：DD_todc(0)
  返回 -1 是正常行为，不构成失败判据——正确初始化调用是 DD_btn(0)）。
  万象连点器实际用的是自带的 DD 40800 老免费版（运行时动态创建 DD40800 服务、
  释放-使用-自毁，无持久文件），与本 DLL 无关。排除决策不变，原因修正。
  若用户机器只有该版本需另行获取 x64 免费版
